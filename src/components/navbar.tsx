...
function CasinoIndexIcon({ className, inverted }: { className?: string; inverted?: boolean }) {
  return (
    <svg
      width="19"
      height="20"
  ...
  return (
    <header
      ...
      className={`sticky top-0 z-40 transition-transform duration-300 ${
        isDark ? "bg-[#060505] border-b border-white/10" : "bg-white border-b border-neutral-100"
      } ${visible ? "translate-y-0" : "-translate-y-full"} ${
        !atTop && visible ? (isDark ? "shadow-lg" : "shadow-sm") : ""
      }`}
    >
      <nav
        data-name="nav-bar"
      ...
      >
        ...
        >
          ...
          <NavDropdown
            trigger={
              <button className={`flex items-center gap-1.5 text-sm font-medium transition-colors rounded-lg px-3 py-1.5 outline-none cursor-pointer ${
                ...
                <Image src="/icons/casino.svg" alt="" width={19} height={20} className={`shrink-0 ${isDark ? "invert" : ""}`} />
          ...
          {navCategories.filter((item) => item.href).map((item) => {
            ...
            return (
              ...
              >
                <Image src={item.icon} alt="" width={19} height={20} className={`shrink-0 ${isDark && !isActive ? "invert" : ""}`} />
        ...
        <Link href="/" className="absolute left-1/2 -translate-x-1/2">
          <Image
            src={isDark ? "/icons/logo-white.svg" : "/icons/logo.svg"}
            alt="CoinBets"
            width={135}
            height={32}
        ...
        <div data-section="nav-actions" className="flex items-center gap-5">
          ...
          >
            ...
            >
              <Search className={`size-6 ${isDark ? "text-white" : "text-neutral-600"}`} />
            </button>
